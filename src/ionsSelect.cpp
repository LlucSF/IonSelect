...
void IonsSelect::setThreshold(double threshold)
{
/*    
  if(threshold<=0) 
...
int IonsSelect::getMeasures()
  ...
  double pixelValue;
  IonsTreat ionsTreat(m_threshold);
...
int IonsSelect::setBoundaries(double percentil)
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int IonsSelect::setBoundaries(double zPercentil, double pPercentil, double fcPercentil)
  {
  m_zProb=zPercentil/100.0;
  m_pProb=pPercentil/100.0; 
  m_fcProb=fcPercentil/100.0;
  setZPFcBoundaries();
  return 0;
  }
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int IonsSelect::setZPFcBoundaries()
{
  int pCol, pGr, sGr;
...
//  int eCount[3];
  ...
  prob[0]=m_zProb; //los valores deben estar establecidos antes (en setBoundaries())
  prob[1]=m_pProb;
  prob[2]=m_fcProb;
 ...
 for(int ion=0; ion<m_pTestRows; ion++) //para todos los iones
  ...
  for(int i=0; i<3; i++)
    ...
    if(hSize[i]==0)
        hSize[i]=m_nPTestGroups*m_nPTestGroups*m_pTestRows; //maximo posible (evita div por cero)
    }
...
int IonsSelect::ionsSelectToFile(const char* fileName, int *grTest_p, int nGrTest, int ordination)
{
  ...
  fprintf(fp, "\tThreshold value for Z:%.3e\r\n",  m_threshold);  
  fprintf(fp, "\tPercentiles: Z=%.2f; P=%.2f; FC=%.2f\r\n", m_zProb*100.0, m_pProb*100.0, m_fcProb*100.0); 
  ...
  if(ordination==0)
    fprintf(fp, "  Ordination: distance parameter\r\n");
  else
    fprintf(fp, "  Ordination: contrast parameter\r\n");
...
//	    fprintf(fp, "%+.4f %+.1e %+.4f ", Z, P, FC);      
  ...
  fprintf(fp2, "\tThreshold value for Z:%.3e\r\n",  m_threshold);  
  fprintf(fp2, "\tPercentiles: Z=%.2f; P=%.2f; FC=%.2f\r\n", m_zProb*100.0, m_pProb*100.0, m_fcProb*100.0); 
...
int IonsSelect::getAllUpRegulatedNumber()
  {
  int count_14=0;   
  //para todos los grupos analizados
  for(int focalGroup=0; focalGroup<m_nAnalyzeGr; focalGroup++)
    {
    //Se cuentan los iones upregulated (ions_14)
    for(int ion=0; ion<m_pTestRows; ion++) //para todos los iones
      {
      if( m_ionsClusterZ_p[ion][focalGroup]==1 || m_ionsClusterV_p[ion][focalGroup]==4)
	count_14++;
...
int IonsSelect::getAllDownRegulatedNumber()
  {
  int count_23=0;   
  //para todos los grupos analizados
  for(int focalGroup=0; focalGroup<m_nAnalyzeGr; focalGroup++)
    {
    //Se cuentan los iones downregulated (ions_23)
    for(int ion=0; ion<m_pTestRows; ion++) //para todos los iones
      {
      if( m_ionsClusterZ_p[ion][focalGroup]==3 || m_ionsClusterV_p[ion][focalGroup]==2)
	count_23++;
